...
If ANY check fails, call create_xlsx again with corrections.`,
    ...
    run_in_background: {
        ...
        description: 'Delegate a self-contained research sub-question to a READ-ONLY background task that runs alongside this conversation. The report lands as a new conversation in the history. Single slot: only one background task at a time.',
        example: 'run_in_background("Survey all notes tagged #philosophy and summarize the main threads", "Philosophy survey")',
        whenToUse: 'Broad research that would block the chat for minutes and whose result THIS conversation does not need for its next step.',
        commonMistakes: 'Using it for writes (it is read-only) or for results the current turn depends on (it returns immediately, not with the answer).',
    },
    consult_flagship: {
